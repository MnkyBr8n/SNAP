...
- Parser tracking (tree_sitter, semgrep, nim_parser)
"""
...
def _build_warning_handler(log_dir: Path) -> logging.Handler:
    ...
    handler = logging.FileHandler(
        str(log_dir / "app.log"), encoding='utf-8', delay=True
    )
...
def _build_debug_handler(log_dir: Path) -> logging.Handler:
    """Build INFO+ handler → app_debug.log (5 MB × 3 rotating)."""
    ...
    handler = logging.handlers.RotatingFileHandler(
        str(log_dir / "app_debug.log"), maxBytes=5 * 1024 * 1024, backupCount=3,
