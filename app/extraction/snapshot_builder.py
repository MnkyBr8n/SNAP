...
class SnapshotBuilder:
    ...
    def _validate_template(self, snapshot_type: str, template: Dict[str, Any]) -> bool:
        """Warn on unregistered fields but accept the template."""
        registered_ids = self._registered_field_ids()
        unknown_fields = [f for f in template.get("fields", {}) if f not in registered_ids]
        if unknown_fields:
            self.logger.warning(
                f"Template '{snapshot_type}' has unregistered fields (ignored): {unknown_fields}",
    ...
    def create_snapshots(
        ...
        for snapshot_type, fields in categorized_fields.items():
            ...
            # Fill in missing template fields with empty defaults
            complete_fields = dict(fields)
            if template:
                for field_id in template.get("fields", {}):
                    if field_id not in complete_fields:
                        complete_fields[field_id] = None
            else:
                complete_fields = fields

            # Compute three-layer hashes
            field_text = json.dumps(complete_fields, sort_keys=True)
