...
def valid_snapshot_types() -> set:
    """
    Derive valid snapshot types from the intersection of:
    - master_notebook.yaml snapshot_templates keys
    - *.json files present in schemas/snapshot_templates/
    Both must agree — a type is only valid if registered in the YAML and has a JSON file on disk.
    """
    import yaml
    settings = get_settings()
    ...
    if master_path.exists():
        try:
            ...
            yaml_types = set(master.get("snapshot_templates", {}).keys())
        except (OSError, yaml.YAMLError, UnicodeDecodeError):
            pass

    json_types: set = set()
    if templates_dir.exists():
        json_types = {p.stem for p in templates_dir.glob("*.json")}

    return yaml_types & json_types

def validate_snapshot_type(snapshot_type: str) -> str:
    valid = valid_snapshot_types()
    if valid and snapshot_type not in valid:
        raise ValidationError(f"Invalid snapshot_type: {snapshot_type!r}")
