...
NOT_ALLOWED_ACTIONS: frozenset[str] = frozenset({
    ...
    "ingest_files",         # SNAP ingests — LLM has no role in ingest decisions
})
...
async def handle_query_snapshots(
    ...
    """
    [ALLOWED] Query DB snapshots by type and file. Both filters are required.

    Args:
        project_id: Project identifier
        snapshot_type: Filter by snapshot type (e.g., "security", "imports")
        file_path: Filter by source file path

    Returns:
           Matching snapshot for the given file and type
    """
    project_id = validate_project_id(project_id)
